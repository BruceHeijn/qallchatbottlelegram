...
# Фразы для roast (agr)
roast_phrases = [
    "{name}, ты словно стиралка, в которой уже никто не застрянет.",
    "{name}, твои победы такие же редкие, как Wi-Fi в деревне.",
    "{name}, если бы тупость светилась, ты был бы городом-миллионником.",
    "{name}, ты как я — дрочишь.",
    "{name}, когда мозги раздавали, ты в очереди за мемами стоял.",
    "{name}, с тобой скучно даже тайским шлюхам.",
    "{name}, ты словно финал Игры Престолов — все ждали большего, а получили тебя.",
    "{name}, твой интеллект — как 404: не найден.",
    "{name}, ты как патч без документации — никто не знает, зачем ты нужен.",
    "{name}, твоя харизма настолько мала, что даже боты в чате тебя игнорят.",
    "{name}, ты как биткоин в 2018 — все надеялись, а ты обвалился.",
    "{name}, твои шутки такие же смешные, как синий экран смерти.",
    "{name}, ты словно баг в коде — раздражаешь всех, но никто не хочет фиксить.",
    "{name}, если бы лень была олимпийским видом спорта, ты бы взял золото.",
    "{name}, ты как спам в почте — никому не нужен, но всё равно лезешь.",
    "{name}, твоя жизнь — как подписка на Netflix: никто не знает, за что платит.",
    "{name}, ты как старая Nokia — вроде легенда, но никому не нужен в 2025.",
    "{name}, твои планы такие же стабильные, как интернет в метро.",
    "{name}, ты как кофе без кофеина — вроде есть, а толку ноль.",
    "{name}, если бы был конкурс на самый бесполезный баг, ты бы выиграл с отрывом.",
    "{name}, ты как Windows Vista — все хотят забыть, что ты существуешь.",
    "{name}, твоя продуктивность — как батарея на 1%: вот-вот сдохнешь.",
    "{name}, ты как QR-код без интернета — все смотрят, но никто не понимает.",
    "{name}, твои идеи такие же свежие, как хлеб в супермаркете в понедельник.",
    "{name}, ты как спойлер к фильму — портишь всё, ещё не начав.",
    "{name}, если бы был налог на скуку, ты бы уже был банкротом.",
    "{name}, ты как лагающий стрим — все хотят посмотреть, но лучше выключить.",
]
...
epic_phrases = [
    ...
    "Свет звёзд озарил {handsome}, а {not_handsome} утонул в тени провала.",
    "Боги выбрали {handsome} для славы, а {not_handsome} — для мемов.",
    "В эпической саге дня {handsome} — король, а {not_handsome} — шут.",
    "{handsome} сияет, как алмаз, а {not_handsome} — как уголь в подвале.",
    "В летописях чата запишут: {handsome} — легенда, {not_handsome} — анекдот.",
    "{handsome} — как меч в камне, а {not_handsome} — как камень в ботинке.",
    "Судьба улыбнулась {handsome}, а {not_handsome} она просто ткнула пальцем и засмеялась.",
    "{handsome} — герой эпоса, а {not_handsome} — его комическая вставка.",
    "Даже в Матрице {handsome} был бы Нео, а {not_handsome} — баг в коде.",
    "{handsome} взлетел к звёздам, а {not_handsome} застрял в земной грязи.",
    "В этой истории {handsome} — рыцарь, а {not_handsome} — его оруженосец, уронивший щит.",
    "{handsome} — как Илон Маск на запуске, а {not_handsome} — как Tesla на отзыве.",
    "Сага гласит: {handsome} правит, а {not_handsome} падает в лужу.",
    "{handsome} — это эпик вин, а {not_handsome} — эпик фейл.",
    "В битве за славу {handsome} взял трофей, а {not_handsome} — только лайки за стыд.",
    "{handsome} — как викинг на драккаре, а {not_handsome} — как его утонувший весло.",
    "Даже в космосе {handsome} был бы звездой, а {not_handsome} — космическим мусором.",
    "{handsome} пишет историю, а {not_handsome} — только комментарии к ней.",
    "В этот день {handsome} стал легендой, а {not_handsome} — её сноской.",
    "{handsome} — как супергерой на экране, а {not_handsome} — как его дублёр, упавший со сцены.",
]
